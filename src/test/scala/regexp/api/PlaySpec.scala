...
class PlaySpec extends FlatSpec with Matchers {
  ...
  "Matcher" should "pass Act II, Scene III" in {
    ...
    def seqn(n: Int, r: State[Char, Boolean]) = {
      List.fill(n)(r).reduce(sequence(_, _))
    }

    def re[C](n: Int) = sequence(seqn(n, alternative(a, Builder.empty())), seqn(n, a))

    val n = 10

    Matcher.run(re(n), List.fill(n)('a')) should be(true)
  }
  ...
  //
  //    lazy val anbn: State[Char, Boolean] = alternative(Builder.empty(), sequence(a, sequence(anbn, b)))
  //
  ...
  //
  //    def abc(n: Int): State[Char, Boolean] = sequence(a, alternative(bcs(n), abc(n + 1)))
  //
